...
data_plot3<- data_plot |> 
 ...
 plot2 <- ggplot(data_plot2) +
   geom_sf(aes(fill = percentage)) +
   geom_sf_text(aes(label = postal), size=3, color="white", check_overlap = TRUE)+
   coord_sf(
     crs = st_crs(3467), 
 ...
 ggdraw(plot1) +
   draw_plot(plot2, width = 0.18, height = 0.18 * 10/6 * ratioAlaska, 
             x = 0.10, y = 0.05) +
   draw_plot(plot3, width = 0.11, height = 0.11 * 10/6 * ratioHawaii, 
             x = 0.28, y = 0.05)
